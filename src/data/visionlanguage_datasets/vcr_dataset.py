...
class VCRDataset(Dataset):
    ...
    def __init__(self,
        ...
        if os.path.exists(self.cached_data_file):
            ...
            for line in tqdm(json_lines):
                ...
                image_path = os.path.join('drawn_images/' + str(split) + '/' + str(task_type)+ '/' + str(line['annot_id']) +'.jpg')  ## train-0, train-1, train-2
                multichoice_texts = []
    ...
    def convert_to_low_shot(self, low_shot_percentage: float):
        ...
        new_data = random.Random(1).sample(self.data, n_low_shot_examples)
        self.data = new_data
        self.n_examples = len(self.data)
...
def build_vcr_dataloader(args,
    ...
    logger.info("Creating VCR {} dataloader with batch size of {}".format(split, args.batch_size//4))
