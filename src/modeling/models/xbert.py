...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        if hasattr(config, 'adapter_config'):
            self.adapter = Adapter(**config.adapter_config, model_dim=config.hidden_size)

    def forward(self, hidden_states, input_tensor):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        if hasattr(self, 'adapter'):
            hidden_states = self.adapter.adapter_layer_forward_bert(hidden_states, input_tensor, self.LayerNorm)
        else:
            hidden_states = self.LayerNorm(hidden_states + input_tensor)
        return hidden_states
...
class BertLMHeadModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        return CausalLMOutputWithCrossAttentions(  # prediction_scores: [batch_size, seq_len(num_words), vocab_size(30522)]
            loss=lm_loss,
