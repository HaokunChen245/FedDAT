...
from src.modeling.adaptered_output import Adaptered_ViltOutput, Adaptered_BertOutput
...
class ViltBertContinualLearner(ContinualLearner):
    ...
    def add_adapter(self):
        for i in range(len(self.viltbert_encoder.vilt.encoder.layer)):
            self.viltbert_encoder.vilt.encoder.layer[i].output = Adaptered_ViltOutput(
                self.viltbert_encoder.vilt.encoder.layer[i].output,
                self.adapter_config,
            )

        # for i in range(len(self.viltbert_encoder.bert.encoder.layer)):
        #     self.viltbert_encoder.bert.encoder.layer[i].output = Adaptered_BertOutput(
        #         self.viltbert_encoder.bert.encoder.layer[i].output,
        #         self.adapter_config,
        #     )
    ...
    def set_active_lora(self):
        ...
        for i in range(len(self.viltbert_encoder.vilt.encoder.layer)):
            in_f, out_f = self.viltbert_encoder.vilt.encoder.layer[i].attention.attention.query.weight.shape[:2]
            self.viltbert_encoder.vilt.encoder.layer[i].attention.attention.query = lora.Linear(in_f, out_f, r=16)
            self.viltbert_encoder.vilt.encoder.layer[i].attention.attention.value = lora.Linear(in_f, out_f, r=16)

        # for i in range(len(self.viltbert_encoder.bert.encoder.layer)):
        #     in_f, out_f = self.viltbert_encoder.bert.encoder.layer[i].attention.self.query.weight.shape[:2]
        #     self.viltbert_encoder.bert.encoder.layer[i].attention.self.query = lora.Linear(in_f, out_f, r=16)
        #     self.viltbert_encoder.bert.encoder.layer[i].attention.self.value = lora.Linear(in_f, out_f, r=16)

    def set_active_adapter(self, name):
        for i in range(len(self.viltbert_encoder.vilt.encoder.layer)):
            self.viltbert_encoder.vilt.encoder.layer[i].output.adapter.set_active_adapter(name)

        # for i in range(len(self.viltbert_encoder.bert.encoder.layer)):
        #     self.viltbert_encoder.bert.encoder.layer[i].output.adapter.set_active_adapter(name)

    def activate_gating(self):
        for i in range(len(self.viltbert_encoder.vilt.encoder.layer)):
            self.viltbert_encoder.vilt.encoder.layer[i].output.adapter.activate_gating()
    ...
    def deactivate_gating(self):
        for i in range(len(self.viltbert_encoder.vilt.encoder.layer)):
            self.viltbert_encoder.vilt.encoder.layer[i].output.adapter.deactivate_gating()

        # for i in range(len(self.viltbert_encoder.bert.encoder.layer)):
        #     self.viltbert_encoder.bert.encoder.layer[i].output.adapter.deactivate_gating()

    def get_param_adapter(self, name):
        l = []
        for i in range(len(self.viltbert_encoder.vilt.encoder.layer)):
            nd = getattr(self.viltbert_encoder.vilt.encoder.layer[i].output.adapter, f'{name}_down')
            l.append(nd.parameters())
            nu = getattr(self.viltbert_encoder.vilt.encoder.layer[i].output.adapter, f'{name}_up')
            l.append(nu.parameters())
            nd = getattr(self.viltbert_encoder.bert.encoder.layer[i].output.adapter, f'{name}_down')
            l.append(nd.parameters())
            nu = getattr(self.viltbert_encoder.bert.encoder.layer[i].output.adapter, f'{name}_up')
            l.append(nu.parameters())
        return l
...
def create_viltbert_continual_learner_model(logger, model_name_or_path: str,
                                            ordered_cl_tasks: List[str],
                                            model_config: Dict,
                                            task_configs: Dict,
                                            device: torch.device):
